!(function (t) {
	...
	let l = document.documentElement.lang.toLowerCase() || 'en',
		c = ['en', 'fr', 'nb', 'no'].find((t) => l.includes(t)) || 'en'
	function h() {
		...
		let t = this.gdprContainer.querySelector('.accept')
		t instanceof HTMLButtonElement && (t.onclick = this.acceptAll)
		let e = this.gdprContainer.querySelector('.customize')
		e instanceof HTMLButtonElement && (e.onclick = () => this.setCustomize(!0))
	}
	let d =
	...
	function g() {
		if (!this.gdprContainer) return
		this.gdprContainer.innerHTML = `<button class="mini-gdpr ${
			this.alignMiniPrompt
		}" data-hide="false" aria-label="${
			this.text?.miniGDPR || 'Cookie settings'
		}"><figure class="icon-cookies settings">${d}</figure></button>`
		let t = this.gdprContainer.querySelector('.mini-gdpr')
		t instanceof HTMLButtonElement && (t.onclick = this.setVisible)
	}
	function u() {
		if (!this.gdprContainer) return
		;(this.gdprContainer.innerHTML = `<div class="pop-up fade-in" lang="${
			document.documentElement.lang
		}"><dialog open>${(function ({ className: t, isOpen: e }) {
			return `<button class="menu-button ${t}" data-open="${e}"><span class="hamburger"><span></span></span></button>`
		})({
			className: 'close-button',
			isOpen: !0,
		})}<div class="dialog-inner-box" style="display:flex;flex-direction:column"><h3><figure aria-label="cookies" class="icon-cookies" style="display:inline-flex;margin-right:.5em">${d}</figure><slot id="customize-header"></slot></h3><p id="customize-text"></p><p id="customize-link"></p><div id="save-wrapper" class="button-wrapper"><button class="button gdpr decline-all" style="background-color:transparent"></button> <button class="button gdpr accept-all"></button></div><div class="button-wrapper">${this.switchButton(
			{
				disabled: !0,
		...
		let t = this.gdprContainer.querySelector('#save-wrapper'),
			e = document.createElement('button')
		;(e.innerText = this.text?.save ?? 'Save preferences'),
			(e.className = 'button gdpr save'),
			(e.onclick = () => {
				this.save(), (this.isCustomize = !1), (this.isVisible = !1)
			})
		let n = this.gdprContainer.querySelector('[name="allowStatistical"]')
		n instanceof HTMLInputElement &&
			(n.onchange = (n) => {
				t?.replaceChildren(e), this.handleChange(n, this)
			})
		let i = this.gdprContainer.querySelector('[name="allowRetargeting"]')
		i instanceof HTMLInputElement &&
			(i.onchange = (n) => {
				t?.replaceChildren(e), this.handleChange(n, this)
			})
		let o = this.gdprContainer.querySelector('.accept-all')
		o instanceof HTMLButtonElement && (o.onclick = this.acceptAll)
		let s = this.gdprContainer.querySelector('.decline-all')
		s instanceof HTMLButtonElement && (s.onclick = this.declineAll)
		let a = this.gdprContainer.querySelector('.close-button')
		a instanceof HTMLButtonElement && (a.onclick = () => this.setCustomize(!1)),
			setTimeout(() => {
	...
	var p =
			...
			n),
		m = (((i = {}).Banner = 'banner'), (i.Box = 'box'), i)
	let b = (t) => {
			if (null != t) return t ? 'granted' : 'denied'
		},
		f = (t) => (null == t ? null : 'granted' === t),
		w = () => {
	...
	function y({ disabled: t = !1, label: e, name: n, value: i }) {
		let o = k()
	...
	let x = Symbol('UPDATE_ON_CONNECTED')
	;('undefined' != typeof window && window.document) ||
		(global.HTMLElement = class {})
	let v = class extends HTMLElement {
			connectedCallback() {
				let t = []
				for (let e of (x in this && Array.isArray(this[x]) && (t = this[x]), t))
	...
	class E extends v {
		connectedCallback() {
			super.connectedCallback(),
				...
				(this.allowStatistical = f(w().analytics_storage)),
				(this.allowRetargeting = f(w().ad_storage)),
				...
				this.googleID?.startsWith('GTM-')
					? ((this._gtm = new _({ consentParams: w(), gtmId: this.googleID })),
					...
					: this.googleID?.startsWith('G-') &&
					  ...
					  this.snapChatPixelID &&
							((this._snapChat = new L({
								snapChatPixelID: this.snapChatPixelID,
							})),
							(this.hasRetargeting = !0)),
					  this.tiktokPixelID &&
							((this._tikTok = new S({ tiktokPixelID: this.tiktokPixelID })),
							(this.hasRetargeting = !0))),
		...
		get alignPrompt() {
			...
			return t && Object.values(p).includes(t) ? t : p.BottomLeft
		}
		...
		get format() {
			...
			return t && Object.values(m).includes(t) ? t : m.Box
		}
		set text(t) {
			this.setText(t), this.setAttribute('text', JSON.stringify(t))
		}
		get text() {
			return JSON.parse(this.getAttribute('text') || 'null')
		}
		...
		save() {
			let t = {
				ad_personalization: b(this.allowRetargeting),
				ad_storage: b(this.allowRetargeting),
				ad_user_data: b(this.allowStatistical),
				analytics_storage: b(this.allowStatistical),
				functionality_storage: 'granted',
				personalization_storage: b(this.allowRetargeting),
				security_storage: 'granted',
			}
			for (let e of (o.set(
				'CookieConsent',
			...
			this._gtm?.updateConsent({ consentParams: w() }),
			this._gTag?.updateConsent({ consentParams: w() }),
			this.allowRetargeting &&
				(this._meta?.initialize(),
				this._snapChat?.initialize(),
				this._tikTok?.initialize()),
		...
		declineAll() {
			;(this.isCustomize = !1),
				...
				this.debug(),
				(window.dataLayer || window.google_tag_data) && location.reload(),
				this.save()
		}
		esc({ key: t }) {
		...
		setText(t) {
			if (!t) return
			let e = this.shadow.querySelector('#cookie-warning-text')
			e && (e.innerHTML = `${t.header} ${d}`)
			let n = this.shadow.querySelector('.customize')
			n instanceof HTMLButtonElement &&
				((n.ariaLabel = t.customize.label), (n.innerText = t.customize.label))
			let i = this.shadow.querySelector('.accept')
			i instanceof HTMLButtonElement &&
				((i.ariaLabel = t.accept), (i.innerText = t.accept))
			let o = this.shadow.querySelector('.accept-all')
			o instanceof HTMLButtonElement &&
				((o.ariaLabel = t.acceptAll), (o.innerText = t.acceptAll))
			let s = this.shadow.querySelector('.settings')
			s instanceof HTMLElement && (s.ariaLabel = t.settings)
			let a = this.shadow.querySelector('.decline-all')
			a instanceof HTMLButtonElement &&
				((a.ariaLabel = t.decline), (a.innerText = t.decline))
			let r = this.shadow.querySelector('#customize-header')
			r instanceof HTMLSlotElement && (r.innerText = t.customize.header)
			let l = this.shadow.querySelector('#customize-text')
			l instanceof HTMLElement &&
				(l.innerHTML = `${t.customize.text}${
					this.hasRetargeting ? ` ${t.customize.retargeting}` : ''
				}`)
			let c = this.shadow.querySelector('#customize-link')
			c instanceof HTMLElement &&
				(c.innerHTML = t.customize.link.replace(
					'%URL%',
					this.privacyPolicyURL || t.policyUrl
				))
			let h = this.shadow.querySelector('.mini-gdpr')
			h instanceof HTMLButtonElement && (h.ariaLabel = t.miniGDPR)
		}
		_addEventListeners() {
		...
		render() {
			;(this.template.innerHTML = '<slot id="gdpr-container"></slot>'),
				(this.shadow.adoptedStyleSheets = [E.styles]),
	...
	'undefined' != typeof window &&
		window.document &&
		!customElements.get('am-gdpr') &&
		customElements.define('am-gdpr', E),
		(t.default = E),
		(t.tagName = 'am-gdpr'),
